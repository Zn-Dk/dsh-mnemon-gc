...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 记忆新鲜度列表：只读视图 + 单条精确删除 + 批量删除 superseded（自定义 modal 确认）。 */
    function FreshnessSection({ connection }) {
      ...
      const bulkDelete = async () => {
        ...
        try {
          const ids = [...selected]
          const res = await connection.rpc.call(FRESHNESS_CHANNEL, "purge-superseded", { ids })
          if (!res.ok) throw new Error(res.error.message)
          setResult(res.value)
          setSelected(new Set())
      ...
      const selectedSuperseded = items?.filter(i => i.superseded && selected.has(i.id)) ?? []
      const selectedNotSuperseded = items?.filter(i => !i.superseded && selected.has(i.id)) ?? []
      ...
      return jsx("div", {
        ...
        children: [
          jsx("div", {
            ...
            children: jsx("div", {
              children: [
                ...
                jsx("p", { children: "只读审阅每条记忆的创建时间、被引用次数与有效重要性；删除按 id 精确执行，不做任何自动分级。批量删除仅限已标记 superseded 的记忆。" }),
              ],
            }),
          }),
          ...
          items !== null && items.length > 0 ? jsx("div", {
            ...
            children: [
              jsx("div", {
                ...
                children: [
                  jsx("span", { className: "gc_listHeadCell", children: "选" }),
                  jsx("span", { className: "gc_listHeadCell", children: "内容" }),
                  jsx("span", { className: "gc_listHeadCell", children: "状态" }),
              ...
              ...items.map((item) => jsx("div", {
                ...
                children: [
                  ...
                  item.superseded
                    ? jsx("span", { className: "gc_listCell", title: item.supersededReason, children: `已取代` })
                    : jsx("span", { className: "gc_listCell", children: "正常" }),
          ...
          confirmOpen ? jsx("div", {
            ...
            children: jsx("div", {
              style: {
                background: "var(--dsw-alias-bg-layer-1)", borderRadius: 12, padding: "20px 24px",
                maxWidth: 480, width: "100%", color: "var(--dsw-alias-label-primary)",
                display: "flex", flexDirection: "column", gap: 12,
              },
              children: [
                jsx("h3", { style: { margin: 0, fontSize: 15, fontWeight: 500 }, children: `确认批量删除 ${selected.size} 条记忆？` }),
                selectedSuperseded.length > 0 ? jsx("div", {
                  style: { display: "flex", flexDirection: "column", gap: 4 },
                  children: [
                    jsx("strong", { style: { fontSize: 12 }, children: "以下记忆已被标记 superseded（可删）：" }),
                    ...selectedSuperseded.map(i => jsx("span", { key: i.id, style: { fontSize: 12, color: "var(--dsw-alias-label-secondary)" }, children: i.content.slice(0, 60) })),
                  ],
                }) : null,
                selectedNotSuperseded.length > 0 ? jsx("div", {
                  style: { display: "flex", flexDirection: "column", gap: 4 },
                  children: [
                    jsx("strong", { style: { fontSize: 12, color: "var(--dsw-alias-state-error-primary)" }, children: "以下记忆未标记 superseded，将被拒绝删除：" }),
                    ...selectedNotSuperseded.map(i => jsx("span", { key: i.id, style: { fontSize: 12 }, children: i.content.slice(0, 60) })),
                  ],
                }) : null,
                jsx("p", {
                  style: { color: "var(--dsw-alias-state-error-primary)", fontSize: 12, margin: 0 },
                  children: "⚠️ 删除不可恢复。仅标记 superseded 的记忆会被删除，其余自动拒绝。",
                }),
                jsx("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end" }, children: [
                  jsx("button", { className: "gc_delBtn", onClick: () => setConfirmOpen(false), children: "取消" }),
                  jsx("button", {
                    className: "gc_delBtn",
                    disabled: deleting || selectedSuperseded.length === 0,
