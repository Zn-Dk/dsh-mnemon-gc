...
export function apply(ctx, config = {}) {
  ...
  ctx.inject(['connection'], (webContext) => {
    ...
    const handler = createSettingsRpcHandler(ctx.settings)
    const maybeDispose = webContext.connection.rpc.handle(SETTINGS_CHANNEL, handler, { authority: 'loopback' })
    if (typeof maybeDispose === 'function') disposeSettingsRpc = maybeDispose
  })
